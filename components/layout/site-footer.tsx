      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <div className="grid gap-10 md:grid-cols-12 md:gap-12">
          <div className="md:col-span-5">
            <Link href="/" aria-label={`Bowling ${branch.city[locale]}`} className="inline-block">
          ...
          <div className="md:col-span-3">
            ...
            <ul className="mt-5 space-y-4 text-sm">
              {branches.map((b) => (
                <li key={b.slug}>
                  <span className="block font-medium text-ink">{b.displayName[locale]}</span>
          ...
          <div className="md:col-span-4">
            ...
            <ul className="mt-5 space-y-3 text-sm text-ink-soft">
              <li className="flex items-start gap-3">
                <IconMapPin className="mt-0.5 size-4 shrink-0 text-ink-muted" aria-hidden />
                <span>{branch.address[locale]}</span>
              </li>
              <li className="flex items-center gap-3">
                <IconPhone className="size-4 shrink-0 text-ink-muted" aria-hidden />
                <a href={`tel:${tel}`} className="hover:text-ink">
                  {branch.phone}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <IconMail className="size-4 shrink-0 text-ink-muted" aria-hidden />
                <a href={`mailto:${branch.email}`} className="hover:text-ink">
                  {branch.email}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <IconBrandWhatsapp className="size-4 shrink-0 text-ink-muted" aria-hidden />
                <a
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-2 border-t border-line pt-6 text-xs text-ink-muted sm:flex-row">
          <span>© {new Date().getFullYear()} Bowling. {t("Footer.rights")}</span>
        </div>
      </div>
    </footer>
  )
